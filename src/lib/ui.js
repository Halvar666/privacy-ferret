// Privacy Ferret user interface controller.
...
const usingAustralis = true; // Pale Moon: prefer ToggleButton API; Widget has no badge
const pWidth = 385;
const pHeight = 560;
...
pbPanel.port.on("openSlideshow", function() {
  tabs.open(data.url("welcome.html"));
  pbPanel.hide();
});
...
function updateCountForChannel(channel, aWin) {
  ...
  if(contentPolicy.Policy.isDisabledRequest(channel, aWin)) {
    if(tab && button && typeof button.state === "function") {
      button.state(tab, {
        icon: {
