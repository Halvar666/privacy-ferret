 ...
 *
 * Privacy Ferret is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 3 as
 * published by the Free Software Foundation.
 *
 * Privacy Ferret is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with Privacy Ferret.  If not, see <http://www.gnu.org/licenses/>.
 */
...
function migrateData(store){
  ...
  try {
    ...
    console.error("privacy ferret: Error writing migration file");
    throw e;
  }
}
 ...
 * cookies for domains where Privacy Ferret has explicitly clobbered/allowed
...
/*
 * Enable Privacy Ferret from the PB panel. Not currently used by itself.
 */
function enable() {
  prefsListener.init();
  userStorage.init();
...
function onContentDocumentGlobalCreated(event){
  ...
  if(host != topHost && cookieblock ){
    ...
    // Also clobber localStorage/sessionStorage in cookieblocked 3rd-party frames.
    subScriptLoader.loadSubScript(self.data.url("clobberlocalstorage.js"), aWin.document);
  }
}

/*
 * Disable Privacy Ferret from the PB panel. This differs from disabling
 * Privacy Ferret from about:addons because the addon button remains in the
...
function main(options) {
  ...
  if (options) {
    ...
    switch(options.loadReason) {
      case "install":
        // Fork note: don't open any first-run page automatically.
        // This avoids linking users to upstream support pages for an unofficial build.
        break;
    }
  }
}
...
// --- Privacy Ferret: local welcome page (Pale Moon) ---
try {
  const tabs = require("sdk/tabs");
  const prefs = require("sdk/simple-prefs").prefs;
  const self = require("sdk/self");

  // Show welcome once per version (install + upgrades)
  if (prefs._ferretWelcomeVersion !== self.version) {
    prefs._ferretWelcomeVersion = self.version;
    tabs.open(self.data.url("welcome.html"));
  }
} catch (e) {}


// --- Privacy Ferret: welcome PageMod (Pale Moon) ---
try {
  const pageMod = require("sdk/page-mod");
  const tabs = require("sdk/tabs");
  const self = require("sdk/self");
  ...
  pageMod.PageMod({
    include: self.data.url("welcome.html"),
    contentScriptFile: self.data.url("welcome-cs.js"),
    onAttach: function(worker) {
      worker.port.on("pf-close-tab", function(){
        try { worker.tab.close(); } catch (e) {}
      });
      worker.port.on("pf-open-options", function(){
        try { tabs.open(self.data.url("options.html")); } catch (e) {}
      });
    }
  });
} catch (e) {}
